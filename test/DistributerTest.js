...
contract("Distributer", async (accounts) => {
    ...
    describe("Checking Founder Allocation type", async () => {
        ...
        let amount = BigInt(400000) * BigInt(10 ** 18);
    ...
    describe("Checking Employee Allocation type", async () => {
        ...
        let amount = BigInt(200000) * BigInt(10 ** 18);
    ...
    describe("Sold Tokens Management", async () => {
        ...
        //     const amount = BigInt(400002) * BigInt(10 ** 18);
        //     await truffleAssert.fails(distributer.soldTokensManagement(buyer, amount));
            
        // });


        it("The owner can  sale tokens", async () => {

            const amount = BigInt(400000) * BigInt(10 ** 18);
            await distributer.soldTokensManagement(buyer, amount);
    ...
    describe("Claim", async () => {
        ...
        it("Claiming by employee will be succsessful using time travel", async () => {
            ...
            assert.equal(BigInt(result.claimed),BigInt(result.totalAmount));
        ...
        it("The employee claim in random mounths", async () => {
            ...
            assert.equal(BigInt(user.claimed),BigInt(user.totalAmount));
            console.log(`The employee claim in month ${ rand }: ${BigInt(user.claimed)}`);
        });
    */
        /*
    ...
    */
        describe("Final balance", async () => {
       
          
           
           it("Balance must be 0", async () => {
              
            let balance = BigInt(await fastToken.balanceOf(distributer.address));
           
               console.log(balance);
               await assert(balance == 0,"Balanse mast be 0");
           });
       });
